...
export default function ResultsPage() {
  ...
  const buttonColor = isSuccess ? "bg-[#1e7f43] hover:bg-green-800" : "bg-orange-500 hover:bg-orange-600";
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex items-center justify-center p-6">
      <div className={`w-full max-w-2xl rounded-2xl ${bgColor} border-2 ${borderColor} p-8 md:p-12 shadow-lg`}>
        <div className="flex flex-col items-center text-center">
          <CheckCircle className={`w-20 h-20 ${statusColor} mb-6`} />

          <h1 className="text-4xl md:text-5xl font-bold mb-2">Quiz terminé ! 🎉</h1>
          <p className="text-gray-600 text-lg mb-8">Voici tes résultats</p>

          <div className="mb-8 w-full">
            <div className={`inline-block rounded-xl px-8 py-6 ${statusColor}`}>
              <div className="text-6xl font-bold">{percentage}%</div>
              <div className="text-xl mt-2 font-semibold">{score} / {total} bonnes réponses</div>
            </div>
          </div>
          ...
          <div className="w-full mb-8">
            <div className="w-full bg-gray-300 h-3 rounded-full overflow-hidden">
              <div
                className={`h-full transition-all duration-500 ${isSuccess ? "bg-green-500" : "bg-orange-500"}`}
          ...
          <p className={`text-lg font-semibold mb-8 ${statusColor}`}>
            {isSuccess ? "✨ Excellent ! Tu as réussi cette leçon !" : "💪 Continue tes efforts !"}
          </p>
