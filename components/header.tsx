...
export default function Header() {
  ...
  return (
    <header className="flex flex-row items-center justify-between px-8 py-1 shadow-lg sticky top-0 w-full bg-white z-50">
      {/* logo */}
      <div>
        <Image
      ...
      {IsMenuOpen &&(
        <div className="absolute top-full left-0 w-full px-3 py-5 bg-white shadow-md md:hidden">
          ...
          <Link href="/connexion" className="bg-[#1E7F43] hover:bg-[#0f3d22] text-white font-bold py-2 px-4 rounded-xl mt-3 shadow-xl">Se connecter</Link>
        </div>
      )}
    </header>
  );
